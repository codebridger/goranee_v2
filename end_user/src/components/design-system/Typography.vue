...
const tag = computed(() => props.as || (props.variant === 'body' || props.variant === 'caption' ? 'p' : props.variant || 'p'));

const classes = computed(() => {
  const base = 'text-text-primary transition-colors duration-300';
  const variants = {
    h1: 'font-heading font-bold text-5xl md:text-7xl tracking-tight leading-tight',
