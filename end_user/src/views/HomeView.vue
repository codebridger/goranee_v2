...
<template>
	<div class="min-h-screen bg-surface-base text-text-primary transition-colors duration-300 font-sans relative">
		...
		<div class="fixed bottom-4 left-4 z-50 bg-black/80 text-white px-4 py-2 rounded-full text-xs cursor-pointer flex items-center gap-2 hover:bg-black"
		...
		<section class="px-6 py-16 max-w-7xl mx-auto">
			<div class="flex flex-col md:flex-row justify-between items-end mb-10 gap-4">
				<div>
					...
					<Typography variant="body" class="text-text-secondary">Fresh chords added this week</Typography>
				</div>
			...
			<div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8">
				...
				<template v-else>
					<Card v-for="(song, i) in featuredSongs" :key="i" variant="song" hoverable>
						<div class="h-48 rounded-2xl bg-gradient-to-br mb-4 relative overflow-hidden shadow-inner group"
			...
			<div class="max-w-7xl mx-auto px-6">
				<div class="flex justify-between items-center mb-8">
					...
					<a href="#" class="text-text-accent font-bold text-sm flex items-center hover:underline group">
						View All
						<ArrowRight class="w-4 h-4 ml-1 group-hover:translate-x-1 transition" />
					</a>
				</div>
				...
				<div class="flex gap-8 overflow-x-auto pb-8 snap-x snap-mandatory scrollbar-hide">
					<div v-for="(artist, i) in artists" :key="i"
						...
						<div class="w-28 h-28 rounded-full bg-gradient-to-br p-[3px] mb-3 group-hover:scale-110 transition duration-300 shadow-lg relative"
							:class="artist.color">
							<div
						...
						<span class="font-bold text-text-primary group-hover:text-text-accent transition text-lg">{{
							artist.name
							}}</span>
						<span
							class="text-xs text-text-secondary font-medium bg-surface-card px-2 py-0.5 rounded-full shadow-sm mt-1">12
							Songs</span>
					</div>
				</div>
			</div>
